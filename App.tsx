...
function Content() {
  ...
  return <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}><StatusBar style={dark ? "light" : "dark"} />
    ...
    {loading ? <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', gap: 24 }}><Brand /><ActivityIndicator color={C.accent} /></View> : route.code || route.chatId ? <Found token={token} key={route.code || route.chatId} code={route.code} chatId={route.chatId} goHome={() => navigate({})} goChat={id => navigate({ chatId: id })} /> : token && user ? <Dashboard key={user.id} token={token} user={user} onUserUpdated={setUser} onLogout={logout} onScan={() => setScan(true)} onHelp={() => setHelp(true)} onExpired={() => { void tokenStorage.clear(); setToken(null); setUser(undefined); setError(t("Sua sessão terminou. Entre novamente para continuar.")); }} /> : <Auth onAuth={login} onScan={() => setScan(true)} />}
