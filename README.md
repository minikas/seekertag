...
Em 7 de setembro de 2026, o aplicativo foi restaurado à versão `b7ee44d`, anterior às alterações inspiradas no Uniswap. Código, web e pacotes Android/iOS voltaram àquela versão; o banco atual foi preservado. A restauração foi conferida por TypeScript, exportação web e abertura no navegador, sem repetir a bateria de testes cancelada. [Registro da restauração](artifacts/rollback-before-uniswap/verification.json).
...
Validação histórica de 6 de setembro de 2026, anterior às mudanças desfeitas: a suíte foi ampliada porque a rodada anterior tinha 12 testes de API e 8 cenários E2E executados em Chromium; não havia validação iOS/WebKit. Naquela revisão, os 12 testes unitários, 22 testes de API e 34 casos do Playwright passaram: 68 verificações automatizadas, além de dois fluxos Maestro no iOS e um no Android. Esses testes não foram repetidos na restauração de 7 de setembro. Os relatórios produzidos depois dessa versão foram arquivados separadamente e não são atribuídos à fonte restaurada.
