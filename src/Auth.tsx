...
export default function Auth({ onAuth, onScan }: { onAuth: (token: string, user: User, recoveryCode?: string) => Promise<void>; onScan: () => void }) {
  ...
  if (email) return <View style={{ flex: 1 }}><Button variant="ghost" icon="arrow-left" onPress={() => { setEmail(false); setMethods(true); }} style={{ alignSelf: 'flex-start', margin: 12 }}>{t("Todas as formas de entrar")}</Button><EmailAuth onAuth={onAuth} onScan={onScan} /></View>;
  return <View style={{ flex: 1 }}>
    ...
    {methods && <AccountActionSheet title={t("Entrar no SeekerTag")} busy={!!busy} onClose={() => setMethods(false)}>
      ...
      <Pressable accessibilityRole="button" disabled={!!busy} onPress={() => { setMethods(false); setEmail(true); }} style={styles.email}><Text style={styles.emailLabel}>{t("Entrar com e-mail")}</Text><Icon name="arrow-up-right" size={14} color={C.muted} /></Pressable>
    </AccountActionSheet>}
  </View>;
}
const makeStyles = (C: Colors) => StyleSheet.create({
  ...
  center: { textAlign: 'center' }, email: { minHeight: 48, flexDirection: 'row', gap: 8, justifyContent: 'center', alignItems: 'center' }, emailLabel: { color: C.ink, fontSize: 16 },
});
