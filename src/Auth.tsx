...
import { Brand, Button, Icon, Notice, useUI } from './ui';
...
export default function Auth({ onAuth, onScan }: { onAuth: (token: string, user: User, recoveryCode?: string) => Promise<void>; onScan: () => void }) {
  ...
  if (email) return <View style={{ flex: 1 }}><Button variant="ghost" icon="arrow-left" onPress={() => setEmail(false)} style={{ alignSelf: 'flex-start', margin: 12 }}>{t("Todas as formas de entrar")}</Button><EmailAuth onAuth={onAuth} onScan={onScan} /></View>;
  return <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    <View style={styles.content}>
      ...
      <View style={styles.hero}>
        ...
        <Text style={[s.body, { maxWidth: 330 }]}>{t("Entre para cuidar dos seus objetos e facilitar o próximo reencontro.")}</Text>
      </View>
      <View style={styles.methods}>
        <ProviderButton provider="solana" label={t("Continuar com Seeker / Solana")} busy={busy === 'solana'} disabled={!!busy && busy !== 'solana'} onPress={() => void login('solana')} />
        <View style={[s.row, { justifyContent: 'center' }]}><Icon name="shield" size={13} color={C.muted} /><Text style={s.small}>{t("Uma assinatura. Sem senha e sem taxas.")}</Text></View>
        <View style={styles.divider}><View style={styles.line} /><Text style={s.small}>{t("ou continue com")}</Text><View style={styles.line} /></View>
        <ProviderButton provider="google" label={t("Continuar com Google")} busy={busy === 'google'} disabled={!availability || (!!busy && busy !== 'google')} unavailable={availability?.google === false} onPress={() => void login('google')} />
        <ProviderButton provider="apple" label={t("Continuar com Apple")} busy={busy === 'apple'} disabled={!availability || (!!busy && busy !== 'apple')} unavailable={availability?.apple === false} onPress={() => void login('apple')} />
      </View>
      {!!error && <Notice error text={error} />}
      <Text style={[s.small, styles.center]}>{t("Sua conta é criada no primeiro acesso.")}{ '\n' }{t("Depois, é só usar a mesma forma de entrar.")}</Text>
      <Pressable accessibilityRole="button" disabled={!!busy} onPress={() => setEmail(true)} style={styles.email}><Text style={styles.emailLabel}>{t("Entrar com e-mail")}</Text><Icon name="arrow-up-right" size={14} color={C.muted} /></Pressable>
      <View style={styles.finder}>
        <Button variant="secondary" icon="maximize" onPress={onScan} disabled={!!busy}>{t("Encontrei um objeto")}</Button>
        <Text style={[s.small, styles.center]}>{t("Abra a etiqueta e avise o dono.")}{ '\n' }{t("Você não precisa de uma conta.")}</Text>
      </View>
    </View>
  </ScrollView>;
}
