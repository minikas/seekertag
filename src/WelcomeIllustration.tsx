...
function Satellite({ angle, x, y, width, height = width, children }: PropsWithChildren<{ angle: SharedValue<number>; x: number; y: number; width: number; height?: number }>) {
...
export default function WelcomeIllustration({ paused = false }: { paused?: boolean }) {
  ...
  return <View accessible={false} accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={{ width: '100%', aspectRatio: 1 }}>
    ...
    <Animated.View pointerEvents="none" style={[StyleSheet.absoluteFill, orbit]}>
      <Satellite angle={angle} x={35} y={60} width={58}><G transform="rotate(-12 28 28)">
      ...
      <Satellite angle={angle} x={298} y={90} width={54}><G transform="rotate(12 26 26)">
      ...
      <Satellite angle={angle} x={43} y={263} width={54}><G transform="rotate(-8 26 26)">
      ...
      <Satellite angle={angle} x={276} y={280} width={68} height={52}><G transform="rotate(8 26 26)">
