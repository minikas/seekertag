import { Button, C, Field, Icon, Notice, Sheet, s } from '../ui';
...
export function QrScanner({ onScan, onClose }: Props) {
  ...
  return (
    <Sheet title="Ler etiqueta" onClose={onClose}>
      <Text style={s.body}>Aponte a câmera para o QR code. O objeto abrirá aqui, sem precisar de uma conta.</Text>
      <View style={styles.cameraBox}>
        {!permission ? <ActivityIndicator color={C.accent} accessibilityLabel="Verificando permissão da câmera" /> : permission.granted && !cameraFailed ? (
          <>
            <CameraView
          ...
          <View style={styles.permission}>
            ...
            <Text style={[s.h3, { textAlign: 'center' }]}>{cameraFailed ? 'Vamos tentar de outro jeito' : 'Sua câmera lê a etiqueta'}</Text>
            <Text style={[s.small, { textAlign: 'center' }]}>A câmera é usada apenas para ler o QR code. Você também pode colar o link abaixo.</Text>
            {!cameraFailed && <Button onPress={() => void allowCamera()}>{permission.canAskAgain ? 'Permitir câmera' : 'Abrir configurações'}</Button>}
          </View>
        )}
      </View>
      {!!error && <Notice error text={error} />}
      <Field label="Ou cole o link da etiqueta" accessibilityLabel="Link da etiqueta" autoCapitalize="none" autoCorrect={false} keyboardType="url" placeholder="https://…/found/sua-etiqueta" value={manualUrl} onChangeText={setManualUrl} onSubmitEditing={() => accept(manualUrl)} />
      <Button disabled={!manualUrl.trim()} onPress={() => accept(manualUrl)}>Abrir etiqueta</Button>
    </Sheet>
  );
}

const corner = { position: 'absolute' as const, width: 32, height: 32, borderColor: C.accent };
