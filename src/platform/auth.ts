...
export async function authenticate(provider: Provider, mode: AuthMode = 'login', token?: string): Promise<AuthResult | null> {
  if (provider === 'solana') return signInWithWallet(mode, token);
