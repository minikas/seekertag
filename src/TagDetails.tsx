...
export default function TagDetails({ tag, token, user, onClose, onUpdated, onEdit, onTransferred }: Props) {
  ...
  return <Sheet title={page === 'transfer' ? t("Transferir etiqueta") : page === 'info' ? t("Detalhes do objeto") : tag.name} contentKey={page} onClose={close} dismissible={busy !== 'transfer'}
    ...
    {page === 'overview' ? <>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel={t("Recompensa")} onPress={() => onEdit(tag, true)} style={[s.between, s.card]}><RewardSummary reward={tag.reward} amount={tag.rewardAmount} currency={tag.rewardCurrency} /><Icon name="chevron-right" size={20} color={C.muted} /></Pressable>
