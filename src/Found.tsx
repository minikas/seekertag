...
import { categoryInk } from './category.model';
import Conversation from './Conversation';
import { secureStorage } from './platform/storage';
import { Brand, Button, Field, Icon, IconName, Notice, useUI } from './ui';
...
export default function Found({ code, chatId, goHome, goChat }: { code?: string; chatId?: string; goHome: () => void; goChat: (id: string) => void }) {
  ...
  return <KeyboardAwareScrollView bottomOffset={24} style={{ backgroundColor: C.bg, flex: 1 }} contentContainerStyle={{ alignItems: 'center', padding: 22, paddingTop: 30, paddingBottom: 60 }} keyboardShouldPersistTaps="handled"><View style={{ width: '100%', maxWidth: 480, gap: 30 }}><View style={s.between}><Brand /><Button variant="ghost" icon="home" onPress={goHome} label={t("Página inicial")} /></View>
    <View style={[s.card, { padding: 26, gap: 22 }]}>
      {loading && <ActivityIndicator color={C.accent} />}
      {!!error && <Notice error text={error} />}
      {chatId && chatToken ? <><View style={{ gap: 9 }}><Text accessibilityRole="header" style={s.h2}>{t("Obrigado por ajudar.")}</Text></View><Conversation id={chatId} token={chatToken} finder /></> : tag ? <>
        <View style={{ alignItems: 'center', gap: 16, paddingVertical: 15 }}><View style={[s.circle, { backgroundColor: cat.color, width: 92, height: 92, borderRadius: 28 }]}><Icon name={cat.icon} color={categoryInk(cat.color)} size={39} /></View><Text accessibilityRole="header" style={[s.h1, { fontSize: 30, textAlign: 'center' }]}>{tag.name}</Text><Text style={[s.body, { textAlign: 'center' }]}>{t("Encontrou este objeto? Você pode fazer o dia de alguém melhor.")}</Text></View>
        ...
        {tag.rewardAmount > 0 && <View style={{ padding: 15, borderRadius: 12, backgroundColor: C.amberSoft, gap: 8 }}><View style={s.row}><Icon name="gift" color={C.amber} size={18} /><Text style={{ fontWeight: '600', color: C.amber }}>{tag.rewardCurrency === 'BRL' ? 'R$' : tag.rewardCurrency} {tag.rewardAmount.toLocaleString(locale)} {t("de agradecimento")}</Text></View><Text style={s.small}>{t("Promessa do dono, combinada após a devolução. O SeekerTag não guarda ou transfere esse valor.")}</Text></View>}
        <Field label={t("Como podemos te chamar? (opcional)")} value={finderName} onChangeText={setFinderName} placeholder={t("Seu primeiro nome ou apelido")} maxLength={60} />
        <Field label={t("Mensagem para o dono")} value={message} onChangeText={setMessage} placeholder={t("Oi! Encontrei seu objeto. Podemos combinar a devolução por aqui.")} multiline maxLength={2000} />
        <Button onPress={submit} busy={busy} icon="send">{t("Avisar o dono")}</Button>
        <View style={[s.row, { alignItems: 'flex-start' }]}><Icon name="shield" size={16} color={C.accent} /><Text style={[s.small, { flex: 1 }]}>{t("A conversa acontece aqui. Seu telefone, e-mail e localização não são pedidos nem compartilhados automaticamente.")}</Text></View>
      </> : null}
    </View>
