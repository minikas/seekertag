...
import { invalidateApiMutation } from './query-cache';
...
export async function api<T>(path: string, token?: string | null, body?: unknown, method?: string, signal?: AbortSignal): Promise<T> {
  ...
  try {
    const response = await fetch(`${API_URL}${path}`, { method: verb, headers: { ...(body !== undefined ? { 'Content-Type': 'application/json' } : {}), ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: body === undefined ? undefined : JSON.stringify(body), signal: controller.signal });
    const data = response.status === 204 ? undefined : await response.json();
    if (!response.ok) throw new ApiError(data?.error || 'Não foi possível concluir. Tente novamente.', response.status);
    if (verb !== 'GET' && verb !== 'HEAD') void invalidateApiMutation(token, path).catch(() => {});
    return data;
  } catch (error) {
