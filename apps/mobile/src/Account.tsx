...
export default function Account({ token, user, onUserUpdated, onClose, onHelp, onLogout, onCategoriesChanged }: Props) {
  ...
  const rewardQuery = useQuery(apiQueryOptions<{ config: RewardConfig | null }>('/rewards/config', token));
  const rewardConfig = rewardQuery.data?.config;
  const sheetRef = useRef<AccountActionSheetHandle>(null);
  const dismissSheet = () => sheetRef.current?.dismiss();
  const logoutMutation = useMutation({ mutationFn: onLogout });
  ...
  async function logout() { if (busy) return; setError(''); try { await logoutMutation.mutateAsync(); } catch (e) { setError((e as Error).message); } }
  return <View style={{ flex: 1 }}>
    ...
    <>
      ...
      <View style={styles.section}>
        ...
        <AccountRow icon="server" title={t("Rede")} value={rewardConfig ? rewardConfig.network : rewardConfig === null ? t("Desativada") : rewardQuery.isError ? t("Indisponível") : t("Carregando…")} onPress={() => setSheet('network')} />
      </View>
      <View style={styles.section}>
    ...
    {rewardQuery.isError && <><Notice error text={rewardQuery.error.message} /><Button variant="secondary" onPress={() => void rewardQuery.refetch()}>{t("Tentar novamente")}</Button></>}
    </Screen>
    </View>
