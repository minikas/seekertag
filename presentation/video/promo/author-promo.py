...
scenes = [
    ...
    dict(id='tag',start=2.4,duration=3.0,title='Scan the QR on the bag.',rule='Rive phone scan'),
    dict(id='message',start=5.4,duration=3.6,title='I found your backpack.',rule='device-frame-stage / touch-indicator'),
    dict(id='chat',start=9.0,duration=3.6,title='Meet at the cafe entrance.',rule='device-frame-stage / touch-indicator'),
...
.reward-stage{top:580px;left:528px;right:80px}.reward-stage .phone{width:440px;padding:10px;border-radius:47px}
.reward-stage .screen{width:416px;height:924.444px;border-radius:34px}
...
.rive-plate{position:absolute;inset:0;width:1080px;height:1920px;object-fit:contain}.scan-title{top:260px;font-size:141px;letter-spacing:-6px}.meeting-note{top:1598px;font-size:38px;color:#00BDCD}.wipe{position:absolute;inset:0;background:#00BDCD;pointer-events:none}
'''
...
for number,s in enumerate(scenes,1):
    ...
    if s['id']=='open':
        body='<div class="scene-surface paper"><video id="bag-open" class="clip rive-plate" src="assets/bag-scan.mp4" muted playsinline data-start="0" data-duration="2.4" data-media-start="0" data-track-index="1"></video><p class="open-tag">YOU KNOW THAT FEELING.</p><h1 class="open-title"><span>WHERE’S</span><span class="question">MY BAG?</span></h1><div class="open-note">Left at the café.</div></div>'
        ...
        body='<div class="scene-surface paper"><video id="bag-scan" class="clip rive-plate" src="assets/bag-scan.mp4" muted playsinline data-start="0" data-duration="3" data-media-start="2.4" data-track-index="1"></video><p class="open-tag">SOMEONE FINDS IT.</p><h1 class="title scan-title"><span>SCAN.</span><span>SAY HELLO.</span></h1></div>'
        js+='tl.fromTo(".scan-title",{y:25,opacity:0},{y:0,opacity:1,duration:.3,ease:"power3.out"},0);tl.to(".scan-title",{y:-18,opacity:0,duration:.25,ease:"power2.in"},2.25);'
    elif s['id']=='message':
        frame,js=phone(sid+'-phone','chat-owner-send',3.8,dur,js,touches=[(5.569,936,2211.24)])
        body=f'<div class="scene-surface paper">{brand("OPEN A PRIVATE CHAT")}<h1 class="title quote"><span>“I found your</span><span>backpack.”</span></h1><div class="phone-stage">{frame}</div><p class="note">Pick a place. Make a plan.</p></div>'
        js+='tl.fromTo(".quote",{y:8},{y:0,duration:.35,ease:"power2.out"},0);'
    elif s['id']=='chat':
        ...
        body=f'<div class="scene-surface">{brand("AGREE ON THE HANDOVER")}<h1 class="title quote"><span>MEET AT</span><span class="accent">THE CAFÉ.</span></h1><div class="phone-stage pair-stage">{owner}{finder}</div><p class="note meeting-note">“I will wait by the entrance.”</p></div>'
        js+=f'tl.fromTo("#{sid}-owner",{{x:110}},{{x:0,duration:.46,ease:"power3.out"}},0);tl.fromTo("#{sid}-finder",{{x:230}},{{x:0,duration:.52,ease:"power3.out"}},0);tl.fromTo(".meeting-note",{{y:18,opacity:0}},{{y:0,opacity:1,duration:.3,ease:"power2.out"}},1.5);'
    elif s['id']=='reward':
        frame,js=phone(sid+'-phone','reward-paid',.75,dur,js,width=416)
        body=f'<div class="scene-surface paper"><video id="reward-animation" class="clip rive-plate" src="assets/finder-reward.mp4" muted playsinline data-start="0" data-duration="3.6" data-track-index="1"></video>{brand("THE RETURN")}<h1 class="title reward-title"><span>BAG BACK.</span><span>SKR RECEIVED.</span></h1><div class="phone-stage reward-stage">{frame}</div><p class="note reward-note">Good deed. Real thanks.</p><p class="devnet">Demo on devnet · test SKR</p></div>'
    ...
    board.append(f'## Frame {number}\nstatus: built\nsrc: compositions/{sid}.html\nStart: {s["start"]}s · Duration: {dur}s\nRules: {s["rule"]}; matched hard cut (directional wipe only at 16.2s; bag/scan is continuous Rive footage).\n{s["title"]}\n')
...
captions=[('Where’s my bag?\nLeft at the café.',0,2.4),('Scan the QR on the backpack.\nSay hello.',2.4,5.4),('“I found your backpack.”',5.4,9),('Meet at the café.\n“I will wait by the entrance.”',9,12.6),('Backpack back. Finder rewarded.\n0.95 test SKR to the finder · Devnet demo',12.6,16.2),('SeekerTag. Lost. Found. Rewarded.\nseekertag.vercel.app',16.2,19.8)]
def stamp(t):
    ms=round(t*1000);return f'{ms//3600000:02}:{ms//60000%60:02}:{ms//1000%60:02},{ms%1000:03}'
(root/'seekertag-promo-en-vertical.srt').write_text('\n'.join(f'{i}\n{stamp(a)} --> {stamp(b)}\n{text}\n' for i,(text,a,b) in enumerate(captions,1)))
print('Authored bag QR → Rive scan → private chat → meeting point → Rive SKR receipt.')
